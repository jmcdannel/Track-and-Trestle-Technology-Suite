...
from adafruit_httpserver import Server, Route, as_route, Request, Response, GET, POST, PUT, DELETE, JSONResponse
...
def change_neopixel_color_handler_post_json(request: Request):
    ...
    start, end, command, config = data.get("start"), data.get("end"), data.get("command"), data.get("config")
    if command == "color":        
        r, g, b = config.get("r", 0), config.get("g", 0), config.get("b", 0)        
